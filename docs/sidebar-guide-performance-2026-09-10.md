...
decorators and scene strategy with public SDK fake observations. Three tests
passed with explicit runner status, `OK (3 tests)` and final code `-1`:
...
- Repeated sidebar focus navigation; EPG title publication while Guide is hidden
  ...
  programme focus; closing on Channels removes Guide semantics.
...
`lifetime-baseline/`, `shared-width/`, `retained-guide/` each preserve traces,
...
`verify-final-retry.log` preserve attribution and rejected work. Prior
