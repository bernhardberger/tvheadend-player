...
| D16 | Picons are not persisted across restarts; every cold start refetches all over HTSP. Pre-existing. | found | `docs/persistent-cache-plan.md` slice A. |
...
| D19 | Cold start sits on "Loading channel information…" for >6 s (initial EPG sync before the first observation). | device | `docs/persistent-cache-plan.md` slice B. Cold start only. |
...
| D23 | Caching needs an operator "clear cache" control. | user | `docs/persistent-cache-plan.md` slice C. |
