...
internal class Media3PlaybackRuntime(
    context: Context,
    private val client: TvheadendClient,
    private val preferencesProvider: PlaybackPreferencesProvider,
) : PlaybackRuntime {
    private val applicationContext = context.applicationContext
    private val htsp: PlaybackHtspTransport = client.playbackTransport
    private val runtimeJob = SupervisorJob()
    private val mainScope = CoroutineScope(runtimeJob + Dispatchers.Main.immediate)
    ...
    override val state: StateFlow<PlaybackSessionState> = _state

    private val _activeServiceId = MutableStateFlow<Int?>(null)
    override val activeServiceId: StateFlow<Int?> = _activeServiceId
    private val livePlayingServiceTracker = LivePlayingServiceTracker()
    override val playingLiveServiceId: StateFlow<Int?> = livePlayingServiceTracker.serviceId
    private val _activeRecordingId = MutableStateFlow<Int?>(null)
    override val activeRecordingId: StateFlow<Int?> = _activeRecordingId
    ...
    override val timeshiftState: StateFlow<TimeshiftState> = _timeshiftState
    ...
    override val liveSubscriptionFailure: StateFlow<SubscriptionFailureKind?> =
        _liveSubscriptionFailure
    private val _diagnostics = MutableStateFlow(PlaybackDiagnosticsSnapshot())
    override val diagnostics: StateFlow<PlaybackDiagnosticsSnapshot> = _diagnostics
                    ...
                    commands.run {
                        val failure = issuance.commitIfCurrent(recording.generation) {
                            ...
                            recordingPlayerErrorDecision(
                                ...
                                positionMs = playerInstance?.currentPosition ?: 0L,
    ...
    private val diagnosticsListener = object : AnalyticsListener {
        override fun onVideoDecoderInitialized(
            ...
            playerInstance?.videoDecoderCounters?.let { counters ->
    ...
    private fun getOrCreatePlayer(): ExoPlayer {
        ...
        val renderersFactory = LegacyRenderer(applicationContext)
            .setExtensionRendererMode(DefaultRenderersFactory.EXTENSION_RENDERER_MODE_ON)

        return playerInstance ?: ExoPlayer.Builder(applicationContext)
    ...
    @OptIn(UnstableApi::class)
    override suspend fun playLive(serviceId: Int): Boolean {
            ...
            commands.run {
                ...
                val target = withContext(Dispatchers.Main.immediate) {
                    issuance.commitIfCurrent(epoch) {
                        ...
                        consecutiveFailures = 0
                        ActivePlayback(serviceId, epoch).also {
    ...
    override suspend fun playRecording(
        ...
        val availability = recordingPlaybackAvailability(entry)
        require(availability is RecordingPlaybackAvailability.Ready) {
            "Recording ${entry.id} is not available for playback"
        }
    ...
    private suspend fun startRecordingLocked(
        ...
        val recording = withContext(Dispatchers.Main.immediate) {
            issuance.commitIfCurrent(epoch) {
                ...
                val progressCapability = client.recordingProgressCapability.value
                ...
                val progress = RecordingProgressCoordinator(
                    ...
                    writer = RecordingProgressWriter { write ->
                        withContext(Dispatchers.IO) {
                            client.updateRecordingProgress(
                                entryId = entry.id,
        ...
        try {
            val settings = preferencesProvider.currentPreferences()
            if (!issuance.mayCommit(epoch)) return
    ...
    override suspend fun retryRecording(): Boolean {
    ...
    private fun startRecordingRemoteUpdates(recording: ActiveRecording) {
        ...
        recordingRemoteJob = mainScope.launch {
            client.dvrEntries.collectLatest { entries ->
                if (
                    activeRecording?.generation != recording.generation ||
                    !issuance.mayCommit(recording.generation)
    ...
    override fun recordingSeekSettled() {
        ...
        recordingSeekJob = mainScope.launch {
            ...
            val position = playerInstance?.currentPosition ?: return@launch
    ...
    override fun recordingPaused() {
        ...
        val position = playerInstance?.currentPosition ?: return
        mainScope.launch {
            if (
                activeRecording?.generation != recording.generation ||
                !issuance.mayCommit(recording.generation)
    ...
    override suspend fun retryLive(): Boolean {
        if (!manualLiveRetryGate.tryAcquire()) return false
        return try {
            retryLiveCurrent()
        } finally {
    ...
    private suspend fun retryLiveCurrent(): Boolean {
    ...
    override suspend fun release() {
        ...
        try {
            ...
            issuance.failTeardown(epoch, error)
            withContext(NonCancellable) { runtimeJob.cancelAndJoin() }
            throw error
        }
        issuance.completeTeardown(epoch)
        withContext(NonCancellable) { runtimeJob.cancelAndJoin() }
    ...
    override suspend fun pauseTimeshift(): Boolean = withContext(Dispatchers.IO) {
    ...
    override suspend fun resumeTimeshift(): Boolean = withContext(Dispatchers.IO) {
        val source = dataSourceFactory?.currentDataSource ?: return@withContext false
        if (!source.timeshiftState.value.available) return@withContext false
    ...
    override suspend fun goLive(): TimeshiftSeekDecision? = withContext(Dispatchers.IO) {
        val source = dataSourceFactory?.currentDataSource ?: return@withContext null
        if (!source.timeshiftState.value.available) return@withContext null
    ...
    @OptIn(UnstableApi::class)
    override suspend fun setRefreshRateMatchingEnabled(enabled: Boolean) {
        commands.run {
            withContext(Dispatchers.Main.immediate) {
                playerInstance?.setVideoChangeFrameRateStrategy(videoChangeFrameRateStrategy(enabled))
            }
        }
    }
    ...
    private fun resetDiagnosticsCounters() {
        val counters = playerInstance?.videoDecoderCounters
