...
class DvrRepositoryProgressTest {
    ...
    fun ordinaryAndCompletionUpdatesUseExactHtspFields() = runBlocking {
        ...
        val requestOptions = mutableListOf<Triple<String, Long, Boolean>>()
        val repository = repository(
            requestObserver = { method, timeoutMs, disconnectOnTimeout ->
                requestOptions += Triple(method, timeoutMs, disconnectOnTimeout)
            },
        ) { method, fields ->
        ...
        assertEquals(
            RecordingProgressUpdateResult.Accepted,
            repository.updateRecordingProgress(
                entryId = 42,
        ...
        assertEquals(
            listOf(
                Triple("updateDvrEntry", 2_000L, false),
                Triple("updateDvrEntry", 7_654L, false),
            ),
            requestOptions,
        )
    }
    ...
    @Test
    fun invalidSdkRequestDoesNotCallTheService() = runBlocking {
        var requestCount = 0
        ...
        assertEquals(
            ...
            repository.updateRecordingProgress(-1, 0L, setWatched = false),
        )
        assertEquals(
            RecordingProgressUpdateResult.Rejected,
            repository.updateRecordingProgress(0, -1L, setWatched = false),
        )
    ...
    @Test
    fun transportAndExecutionFailuresAreTypedWithoutRequestDisconnectPolicy() = runBlocking {
        ...
        assertEquals(
            RecordingProgressUpdateResult.Disconnected,
            repository { _, _ -> throw IOException("closed") }
                .updateRecordingProgress(1, 180L, false),
        )
        assertEquals(
            RecordingProgressUpdateResult.Disconnected,
            repository { _, _ -> throw IllegalStateException("Not connected") }
                .updateRecordingProgress(1, 180L, false),
        )
        assertEquals(
            RecordingProgressUpdateResult.Rejected,
            repository { _, _ -> throw IllegalStateException("wrong state") }
                .updateRecordingProgress(1, 180L, false),
        )
        assertEquals(
            RecordingProgressUpdateResult.Rejected,
            repository { _, _ -> throw RuntimeException("rejected") }
                .updateRecordingProgress(1, 180L, false),
        )
    }
