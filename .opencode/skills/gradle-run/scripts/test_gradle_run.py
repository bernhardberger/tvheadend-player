...
# Based on chrisbanes/skills@ded78abbe5a170c9ca0497b614f63c1a872d9f8e.
# TVHeadend Player locally added focused live-test credential isolation coverage.
...
class CommandEnvironmentTest(unittest.TestCase):
    def test_live_test_credentials_are_removed_by_default(self) -> None:
        with mock.patch.dict(
            os.environ,
            {
                "TVHEADEND_SOAK_CREDENTIALS_FILE": "/private/credentials.json",
                "TVHEADEND_SOAK_NODVR_CREDENTIALS_FILE": "/private/no-dvr.json",
            },
            clear=True,
        ):
            environment = GRADLE_RUN.command_environment()

        self.assertNotIn("TVHEADEND_SOAK_CREDENTIALS_FILE", environment)
        self.assertNotIn("TVHEADEND_SOAK_NODVR_CREDENTIALS_FILE", environment)

    def test_explicit_live_test_opt_in_preserves_credentials(self) -> None:
        with mock.patch.dict(
            os.environ,
            {
                "GRADLE_RUN_ALLOW_LIVE_TESTS": "1",
                "TVHEADEND_SOAK_CREDENTIALS_FILE": "/private/credentials.json",
                "TVHEADEND_SOAK_NODVR_CREDENTIALS_FILE": "/private/no-dvr.json",
            },
            clear=True,
        ...
        self.assertEqual(
            environment["TVHEADEND_SOAK_CREDENTIALS_FILE"],
            "/private/credentials.json",
        )
        self.assertEqual(
            environment["TVHEADEND_SOAK_NODVR_CREDENTIALS_FILE"],
            "/private/no-dvr.json",
        )
